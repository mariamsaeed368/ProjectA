            ...
            if (dataGridView1.CurrentRow.Cells["Id"].Value != DBNull.Value)
            {
                ...
                if (MessageBox.Show("Are You Sure to Delete this Record ?", "DataGridView", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    using (SqlConnection sqlCon = new SqlConnection(conn))
                    {
                        ...
                        cmd1.CommandText = "Delete from  [ProjectA].[dbo].[GroupStudent] where GroupId='" + rowID + "'";
                        cmd1.ExecuteNonQuery();
                        cmd2.CommandText = "Delete from  [ProjectA].[dbo].[GroupProject] where GroupId='" + rowID + "'";
                        cmd2.ExecuteNonQuery();
                        cmd3.CommandText = "Delete from  [ProjectA].[dbo].[GroupEvaluation] where GroupId='" + rowID + "'";
